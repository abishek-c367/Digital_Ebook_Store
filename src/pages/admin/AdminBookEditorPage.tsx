      ...
      supabase
        ...
        .then(({ data, error }) => {
          ...
          setForm({
            ...
            access_type: (data.price_cents || 0) > 0 ? 'paid' : 'free',
            price_cents: data.price_cents || 0,
            currency: data.currency || 'inr',
            razorpay_payment_link: data.razorpay_payment_link || '',
    ...
    try {
      ...
      if (form.access_type === 'paid') {
        if (!form.price_cents || form.price_cents <= 0) {
          showToast('error', 'Paid books need a price greater than zero.');
          setSaving(false);
          return;
        }
        if (!form.razorpay_payment_link.trim()) {
          showToast('error', 'Paid books need a Razorpay Payment Link.');
          setSaving(false);
          return;
        }
      }
      ...
      const payload = {
        ...
        price_cents: form.access_type === 'free' ? 0 : form.price_cents,
        currency: form.currency || 'inr',
        razorpay_payment_link: form.access_type === 'paid' ? form.razorpay_payment_link.trim() : null,
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-6">
        ...
        {/* Access & Pricing */}
        <div className="rounded-xl border border-ink-200 bg-white p-6">
          <h2 className="font-serif text-lg text-ink-900 mb-4">Access & Pricing</h2>

          <div className="mb-5">
            <label className="label-field">Access Type</label>
            <div className="flex gap-3">
              <button
                type="button"
                onClick={() => handleChange('access_type', 'free')}
              ...
              >
                ...
                <span className="block text-xs text-ink-500">Any signed-in reader can open it.</span>
              </button>
              <button
                type="button"
                onClick={() => handleChange('access_type', 'paid')}
                className={`flex-1 rounded-lg border px-4 py-3 text-left text-sm transition-colors ${
              ...
              >
                ...
                <span className="block text-xs text-ink-500">Readers must buy it via Razorpay first.</span>
              </button>
            </div>
          </div>

          {form.access_type === 'paid' && (
            <div className="mb-5 grid gap-4 rounded-lg border border-gold-200 bg-gold-50/40 p-4 sm:grid-cols-2">
              <div>
                ...
                />
                <p className="mt-1 text-xs text-ink-500">In the smallest currency unit (paise for INR, cents for USD).</p>
              </div>
              <div>
              ...
              <div className="sm:col-span-2">
                <label className="label-field">Razorpay Payment Link *</label>
                <input
                  type="url"
                  required={form.access_type === 'paid'}
                  value={form.razorpay_payment_link}
                  onChange={(e) => handleChange('razorpay_payment_link', e.target.value)}
                  className="input-field"
                  placeholder="https://rzp.io/l/xxxxxxxx"
                />
                <p className="mt-1.5 text-xs leading-relaxed text-ink-500">
                  Create this Payment Link in your Razorpay Dashboard (Payment Pages → Payment Links) for the exact
                  price above, then paste its short URL here. This is what the "Buy Book" button opens.
                  In the link's advanced settings, set the <strong>Redirect URL</strong> to{' '}
                  <code className="rounded bg-ink-100 px-1 py-0.5">{`${window.location.origin}/purchase-success`}</code>{' '}
                  so buyers land back here and get instant access after paying. Also add this site's webhook URL
                  in Razorpay Settings → Webhooks (event: <code className="rounded bg-ink-100 px-1 py-0.5">payment_link.paid</code>)
                  as a backup in case the browser redirect doesn't fire.
                </p>
              </div>
            </div>
          )}
