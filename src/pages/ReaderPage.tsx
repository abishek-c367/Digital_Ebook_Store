...
import {
  ...
  ZoomIn, ZoomOut, Loader2, ShieldAlert, BookOpen, Type,
} from 'lucide-react';
import { SEO } from '@/components/SEO';
import { PdfViewer } from '@/components/PdfViewer';
import { verifyAccessToken } from '@/lib/payment';
import { fetchBookById } from '@/lib/books';
import { upsertReadingProgress } from '@/lib/userLibrary';
import { supabase } from '@/lib/supabase';
import type { Book } from '@/types';

type AccessState = 'loading' | 'authorized' | 'denied' | 'expired' | 'invalid';
    ...
    const authorizeReader = async () => {
      ...
      try {
        if (token.startsWith('session:')) {
          ...
          const freeAccessBook = await fetchBookById(bookId);
          if (active && freeAccessBook) {
            setBook(freeAccessBook);
