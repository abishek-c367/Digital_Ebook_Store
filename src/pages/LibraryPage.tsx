...
import { fetchBooks, formatDate, isFreeBook } from '@/lib/books';
import { fetchReadingProgress, fetchUserPurchases } from '@/lib/userLibrary';
...
export function LibraryPage() {
  ...
  useEffect(() => {
    Promise.all([fetchBooks(), fetchUserPurchases()])
      .then(async ([books, purchases]) => {
        const ownedBookIds = new Set(purchases.map((p) => p.book_id));
        // My Library shows books that don't require a purchase, plus
        // whatever paid books this account has actually bought.
        const accessibleBooks = books.filter((book) => isFreeBook(book) || ownedBookIds.has(book.id));

        const enriched = await Promise.all(
          accessibleBooks.map(async (book) => {
      ...
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
            ...
            <h2 className="mt-6 font-serif text-2xl font-light text-ink-900">Your library is empty</h2>
            <p className="mt-2 max-w-sm text-sm text-ink-600">
              Free books you're signed in for and books you've purchased will show up here.
            </p>
